...
[Web Coverage Service (WCS)](https://www.ogc.org/standards/wcs) microservice for [GeoServer Cloud](https://github.com/geoserver/geoserver-cloud). Serves raster coverages (elevation models, satellite imagery, gridded datasets) as GeoTIFF, NetCDF, and other formats.
...
- **Source:** [`src/apps/geoserver/wcs`](https://github.com/geoserver/geoserver-cloud/tree/main/src/apps/geoserver/wcs)
...
The image is built on a GeoServer base layer and includes an **AOT cache** for fast JVM startup. It runs behind the [GeoServer Cloud Gateway](https://hub.docker.com/r/geoservercloud/geoserver-cloud-gateway), which routes `/wcs` requests to it.
...
| --- | --- | --- |
| HTTP port | `9103` | override with `SERVER_PORT` |
| Management port | `8103` | actuator endpoints |
...
| Health | `GET :8103/actuator/health` | liveness & readiness |
| Metrics | `GET :8103/actuator/prometheus` | Prometheus scrape target |
